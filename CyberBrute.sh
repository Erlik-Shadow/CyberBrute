...
#Hash Oluşturma
generate_hash() {
    echo -e "${NEON_CYAN}Hash oluşturulacak metni girin:${NC}"
    read input_text

    echo -e "${NEON_CYAN}Hash algoritmasını seçin:${NC}"
    echo "1) MD5"
    echo "2) SHA1"
    echo "3) SHA224"
    echo "4) SHA256"
    ...
    read algo_choice
    case $algo_choice in
        1) hash=$(echo -n "$input_text" | md5sum | awk '{print $1}') ;;
        2) hash=$(echo -n "$input_text" | sha1sum | awk '{print $1}') ;;
        3) hash=$(echo -n "$input_text" | sha224sum | awk '{print $1}') ;;
        4) hash=$(echo -n "$input_text" | sha256sum | awk '{print $1}') ;;
        5) hash=$(echo -n "$input_text" | sha384sum | awk '{print $1}') ;;
        6) hash=$(echo -n "$input_text" | sha512sum | awk '{print $1}') ;;
        7) 
            if command -v openssl >/dev/null 2>&1; then
                hash=$(openssl passwd -bcrypt "$input_text")
            else
                echo -e "${NEON_PURPLE}Bcrypt için openssl yüklü olmalı!${NC}"
                return
            fi
            ;;
        *)
            echo -e "${NEON_PURPLE}Geçersiz seçim!${NC}"
            return
            ;;
    esac

    echo -e "${NEON_GREEN}Oluşturulan hash: $hash${NC}"
    echo "Hash oluşturuldu: $hash" >> "$REPORT_FILE"
}
...
show_menu() {
    ...
    echo -e "${NEON_GREEN}7. Hash oluşturucu (Metni hash'e çevir)${NC}"
    echo -e "${NEON_GREEN}8. Çıkış${NC}"
    echo -e "${NEON_PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -n -e "${NEON_CYAN}Seçiminizi yapın (1-8): ${NC}"
}
...
while true; do
    ...
    case $choice in
        1|2|3|4)
            # Dosya brute force kısmı (ZIP/RAR/PDF/7z)
            if [ $choice -eq 1 ]; then
                file_type="zip"
        ...
        6)
            ...
            read hash
            echo -e "${NEON_CYAN}Hash türünü girin (md5/ntlm/sha1/sha224/sha256/sha384/sha512/bcrypt):${NC}"
            read hash_type
            hash_type=$(echo "$hash_type" | tr '[:upper:]' '[:lower:]')

            SUPPORTED_HASHES=("md5" "ntlm" "sha1" "sha224" "sha256" "sha384" "sha512" "bcrypt")
            if [[ ! " ${SUPPORTED_HASHES[@]} " =~ " ${hash_type} " ]]; then
                echo -e "${NEON_PURPLE}Hata: Desteklenmeyen hash türü! (md5/ntlm/sha1/sha224/sha256/sha384/sha512/bcrypt)${NC}"
        ...
        8)
            ...
            ;;
        *)
            echo -e "${NEON_PURPLE}Geçersiz seçim! Lütfen 1-8 arasında bir sayı girin.${NC}"
            read -p "Devam etmek için bir tuşa basın..."
            ;;
    esac
done
